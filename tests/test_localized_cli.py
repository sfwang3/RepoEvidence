...
def test_chinese_help_keeps_command_and_option_names() -> None:
    ...
    assert "确定性的、无 LLM 的软件证据收集" in result.output
...
def test_main_preparses_cli_language_before_building_help(
    ...
    assert "确定性的、无 LLM 的软件证据收集" in output
...
def test_main_uses_environment_language_when_cli_option_is_absent(
    ...
    assert raised.value.code == 0
    assert "确定性的、无 LLM 的软件证据收集" in capsys.readouterr().out
...
def test_cli_language_has_priority_over_environment(
    ...
    assert raised.value.code == 0
    output = capsys.readouterr().out
    assert "Deterministic, LLM-free software evidence collection" in output
    assert "确定性的、无 LLM 的软件证据收集" not in output
...
def test_english_and_chinese_report_commands_use_selected_presentation_language(
    ...
    assert "Scan complete. Evidence written to" in english.output
    ...
    assert "报告已生成：" in chinese.output
...
def test_chinese_mysql_error_preserves_canonical_error_code(
    ...
    assert "错误 [mysql_config_missing]：缺少 MySQL 连接配置。" in result.output
